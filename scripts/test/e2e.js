...
async function main() {
  ...
  if (hasNoteEarly && zkEmbeddingResult?.encoded && tweetProofBytesEarly !== "0x" && embeddingProofBytesEarly !== "0x") {
    ...
    try {
      ...
      console.log(`\nTotal Staked in Market: ${ethers.formatEther(totalStaked)} ETH`);
      console.log(`Total Claimed (Payouts+Fees): ${ethers.formatEther(trackerAfterClaims.totalValueClaimed)} ETH`);
      // Check if total claimed <= total staked (allow for potential rounding dust in contract)
      if (trackerAfterClaims.totalValueClaimed <= totalStaked) {
        console.log("✅ Total claimed is less than or equal to total staked.");
      } else {
        console.error("❌ ERROR: Total claimed exceeds total staked!");
      }
      // --- End Claim Tests ---
  ...
  try {
    ...
    // Log individual predictions before claims
    console.log("\n--- Checking Individual Predictions for Market 2 Before Claims ---");
    ...
    for (let predId = 1; predId <= numPredictionsM2; predId++) {
      try {
        ...
        console.log(
          `  Prediction ${predId}: Predictor=${predData.predictor.substring(0, 6)}..., Value=${ethers.formatEther(predData.value)} ETH, IsAgree=${
            predData.isAgree
          }, Timestamp=${new Date(Number(predData.timestamp) * 1000).toISOString()}`
        );
        if (!predData.isAgree) {
          const resolverPred = await resolver.predictions(marketId2, predId);
          console.log(`    -> Disagree Reason: ${resolverPred.reasonText.substring(0, 50)}...`);
        }
      } catch (getPredError) {
        console.error(`   Error fetching prediction ${predId} for Market 2:`, getPredError);
      }
    }
    console.log("--- End Prediction Check ---");

    for (let predId = 1; predId <= numPredictionsM2; predId++) {
      const predData = await market.getPrediction(marketId2, predId);
      const predictorSigner = await ethers.getSigner(predData.predictor);
      try {
        console.log(`Attempting claim for Predictor ${predData.predictor.substring(0, 6)} (Prediction ${predId} - IsAgree=${predData.isAgree})...`);
        const balanceBefore = await ethers.provider.getBalance(predData.predictor);
        ...
        const balanceAfter = await ethers.provider.getBalance(predData.predictor);
        const claimedEvent = claimReceipt.logs.find((log) => log.fragment?.name === "PredictionClaimed");

        if (claimedEvent) {
          const payout = claimedEvent.args.payout;
          console.log(`  ✅ Claimed successfully (Payout=${ethers.formatEther(payout)} ETH)`);
          if (predData.isAgree) {
            // Rough balance check for winners
            const actualChange = balanceAfter - balanceBefore;
            console.log(`     Balance change: ~${ethers.formatEther(actualChange + gasUsed)} ETH (Payout - Gas)`);
          } else {
    ...
    console.log(`\nTotal Staked (Market 2): ${ethers.formatEther(totalStakedM2)} ETH`);
    console.log(`Total Claimed (Market 2): ${ethers.formatEther(trackerM2AfterClaims.totalValueClaimed)} ETH`);
